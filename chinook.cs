namespace chinook {
    ...
    class Chinook {
        public string Main2 (ChinookContext db, TextReader inp) {
            var input_file = inp.ReadToEnd ();
            char[] split_symbol = { '\n', '\r' };
            var input_line = input_file.Split (split_symbol, StringSplitOptions.RemoveEmptyEntries).ToList ();
            var input_db = (IQueryable) db
                .GetType ()
                .GetProperties ()
                .Single (input_dbname => input_dbname.Name == input_line[0])
                .GetValue (db);

            int i = 0;
            while (i < input_line.Count) {
                if (i != 0) {
                    var current_line = input_line[i];
                    var mid_point = current_line.IndexOf (' ');
                    var initial = 0;
                    var operation = current_line.Substring (initial, mid_point);
                    var content = current_line.Substring (mid_point);
                    switch (operation) {

                        case "Select":
                            input_db = input_db.Select (content);
                            break;
                        case "Take":
                            var num_take = Convert.ToInt32 (content);
                            input_db = input_db.Take (num_take);
                            break;
                        case "Skip":
                            var num_skip = Convert.ToInt32 (content);
                            input_db = input_db.Skip (num_skip);
                            break;
                        case "OrderBy":
                            input_db = input_db.OrderBy (content);
                            break;
                        case "Where":
                            input_db = input_db.Where (content);
                            break;
                    }
            ...
            return JsonSerializer.Serialize (input_db.AsEnumerable ().ToList ());
        }
